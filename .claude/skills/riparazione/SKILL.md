...
| "Il modello ha rifiutato la richiesta" | e' un **400 di Groq**, non un rifiuto di contenuto. Nell'ordine: (1) modello ritirato - Groq lo dice con un **400**, non un 404, e `modelloSparito()` deve riconoscerlo o il ripiego non parte; (2) il peso - il limite vero cambia col modello e non e' quello documentato: **non si indovina**, `analizzaScendendo()` riprova piu' leggera quando il server manda `pesante:true`, e `MAX_PAYLOAD_B64` e' solo il punto di partenza. Il messaggio vero sta nei log della function: `Groq 400 su <modello>: <dettaglio>` |
...
| il JSON arriva tagliato a meta' | il ragionamento si mangia i token PRIMA della risposta: `maxTokens` per le immagini in `claude.js` |
